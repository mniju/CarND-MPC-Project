...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //Note: Frst element has all the errors.
    fg[0] = 0;
    
    // The part of the cost based on the reference state.
    for (int t = 0; t < N; t++) {
      fg[0] += 500*CppAD::pow(vars[cte_start + t], 2);//200,500
      fg[0] += 5000*CppAD::pow(vars[epsi_start + t], 2);//200,1000
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 50*CppAD::pow(vars[a_start + t], 2);//5,15
    }
    
    // Minimize the value gap between sequential actuations.
    for (int t = 0; t < N - 2; t++) {
      //Tuning to smooth the steering(from lesson Tuning MPC)
      fg[0] += 500*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);//200,500
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Get the initial value the variables.
 ...
 double latency = 0.1;//100 milliseconds
  ...
  epsi = epsi + v * delta / Lf * latency;
   ...
   // Set the initial variable values to vars.
   //Every other value is set to zero in prev loop.
  vars[x_start] = x;
  vars[y_start] = y;
  vars[psi_start] = psi;
  vars[v_start] = v;
